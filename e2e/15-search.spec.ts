...
test('a channel among the answers opens that channel\'s own screen', async () => {
  const h = await open(WATCH)
  try {
    const ui = app(h.page)
    const over = await openPanel(h.page)
    ...
    // A channel is a place to go now: its videos, newest first, on a screen of
    // its own, with the panel closed behind it.
    const channel = over.locator('.kindRow:has(.avatar)').first()
    await expect(channel).toBeVisible()
    const name = (await channel.locator('.ttl').textContent())?.trim() ?? ''
    await channel.click()
    await expect(over.locator('.modal.search')).toHaveCount(0)
    await expect(ui.locator('.main h2')).toHaveText(name)
    await expect(ui.locator('.main .tile:not([aria-hidden])').first()).toBeVisible({ timeout: 30_000 })
    expect(await h.page.evaluate(() => JSON.parse(localStorage.getItem('oc-easy-mode:state') ?? '{}').view)).toMatch(/^channel:UC/)
  } finally {
    await h.close()
  }
})
